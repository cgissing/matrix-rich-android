...
class MessageItemFactory @Inject constructor(
    ...
    private fun buildItemForTextContent(
        ...
        return if (messageContent.body.shouldRenderAsRichMarkdownBody()) {
            buildRichMarkdownTextItem(messageContent.body, informationData, highlight, callback, attributes)
    ...
    private fun String.shouldRenderAsRichMarkdownBody(): Boolean {
        return containsMarkdownTable() ||
                containsFencedCodeBlock() ||
                containsRawMath() ||
                containsMarkdownBlock() ||
                containsMarkdownEmphasis()
    }

    private fun String.containsMarkdownTable(): Boolean {
        val lines = lineSequence().map { it.trim() }.filter { it.isNotEmpty() }.toList()
        return lines.windowed(2).any { pair ->
            val header = pair[0]
            val separator = pair[1]
            header.count { it == '|' } >= 2 &&
                    Regex("""^\|?\s*:?-{3,}:?\s*(\|\s*:?-{3,}:?\s*)+\|?$""").matches(separator)
        }
    }

    private fun String.containsFencedCodeBlock(): Boolean {
        return contains("```") || contains("~~~")
    }

    private fun String.containsRawMath(): Boolean {
        return Regex("(?s)(?<!\\\\)\\$\\$.+?(?<!\\\\)\\$\\$").containsMatchIn(this) ||
                Regex("(?<!\\\\)(?<!\\$)\\$[^\\n\\$]+?(?<!\\\\)\\$(?!\\$)").containsMatchIn(this)
    }

    private fun String.containsMarkdownBlock(): Boolean {
    ...
    private fun String.containsMarkdownEmphasis(): Boolean {
        return Regex("""(\*\*[^*\n]+\*\*)|(__[^_\n]+__)|(`[^`\n]+`)""").containsMatchIn(this)
    }

    private fun buildRichMarkdownTextItem(
            body: String,
            informationData: MessageInformationData,
            highlight: Boolean,
            callback: TimelineEventController.Callback?,
        ...
        val renderer = htmlRenderer.get()
        val renderedBody = renderer.renderRichMarkdown(body, pillsPostProcessor)
        return buildMessageTextItem(
                renderedBody,
                true,
                informationData,
                ...
                callback,
                attributes,
                markwonPlugins = renderer.richMarkdownPlugins,
        )
    }
    ...
    private fun buildMessageTextItem(
            ...
            markwonPlugins: List<MarkwonPlugin> = htmlRenderer.get().plugins,
    ): MessageTextItem? {
