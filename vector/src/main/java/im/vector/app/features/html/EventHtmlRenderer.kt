...
class EventHtmlRenderer @Inject constructor(
    ...
    private val richMarkdownPreprocessor = object : AbstractMarkwonPlugin() {
        override fun processMarkdown(markdown: String): String {
            return markdown.replace(INLINE_MATH_REGEX) { matchResult ->
                "${'$'}${'$'}${matchResult.groupValues[1]}${'$'}${'$'}"
            }
        }
    }
    ...
    private fun buildRichMarkdownMarkwon() = Markwon.builder(context)
            .usePlugin(richMarkdownPreprocessor)
            .usePlugin(HtmlRootTagPlugin())
            .usePlugin(HtmlPlugin.create(htmlConfigure))
    ...
    get() {
        ...
        if (changed) {
            field = buildMarkwon()
            richMarkdownMarkwon = buildRichMarkdownMarkwon()
        }
        return field
    }

    private var richMarkdownMarkwon: Markwon = buildRichMarkdownMarkwon()

    val plugins: List<MarkwonPlugin>
        get() = markwon.plugins

    val richMarkdownPlugins: List<MarkwonPlugin>
        get() = richMarkdownMarkwon.plugins
    ...
    fun renderRichMarkdown(text: String, vararg postProcessors: PostProcessor): CharSequence {
        return try {
            val parsed = richMarkdownMarkwon.parse(text)
            val renderedText = richMarkdownMarkwon.render(parsed).toSpannable()
            postProcessors.forEach {
                it.afterRender(renderedText)
            }
            renderedText
        } catch (failure: Throwable) {
            Timber.v(failure, "Fail to render rich markdown ${text.redactIfNotDebug()}")
            text
        }
    }
